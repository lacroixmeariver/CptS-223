...
protected:
	...
	void printLevelOrderHelper(Node<T>* root, std::ostream& out) {
		...
		outputQueue.push(root); // push root address into queue
		while (!outputQueue.empty()) 
		{
			Node<T>* tempNode = outputQueue.front(); // tempNode is a copy of the address in the front of the queue 
			out << tempNode->value << " "; // processing first part of subtree 
			outputQueue.pop(); // processed, leave queue

			if (tempNode->left != nullptr) // if there is a left child
			{
				outputQueue.push(tempNode->left); // add to queue
			}
			if (tempNode->right != nullptr) // if there is a right child 
			{
				outputQueue.push(tempNode->right); // add to queue
			}
			// as it repeats, the front of the queue will always be the root of the left subtree followed by the root of the right
		}
...
public:
	...
	BST(BST&& other) : _root(nullptr) {
		...
		_root = cloneTree(other._root); // making copy of the other tree 
		other._root = nullptr;
	...
	BST& operator=(BST&& other) {
		...
		_root = other._root; // switching over the root 
		other._root = nullptr;  // releasing the data of the other tree
		return *this;
	}
