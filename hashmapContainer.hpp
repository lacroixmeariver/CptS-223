...
class Hashmap
{
    ...
    void rehash()
    {
        ...
        Array<Node*> tempContainer = mapContainer; // saving the contents of the old container 
        mapContainer.clear(); // clearing main container out 
        mapContainer = Array<Node*>(capacity); // in with the new one double the size 
    ...
    void insert(K keyVal, T dataVal)
    {
        ...
        Node* current = mapContainer[index]; // keeping track of current node for separate chaining 
        ...
        if(current != nullptr && loadFactor() < 0.5) // if there's something in the same index but there's room for it in the map
        {
